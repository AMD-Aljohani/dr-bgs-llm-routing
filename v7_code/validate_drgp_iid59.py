...
ROOT = Path(__file__).resolve().parents[1]; OUT=ROOT/'v7_results'/'iid59_drgp'; OUT.mkdir(parents=True,exist_ok=True)
...
rows=[]
for p in sorted(glob.glob(str(ROOT/'v7_results/iid59/I*_policy_surface.csv'))):
...
raw=pd.DataFrame(rows); raw.to_csv(OUT/'iid59_drgp_runs.csv',index=False)
...
per.to_csv(OUT/'iid59_drgp_by_scenario.csv',index=False)
...
pd.DataFrame([summary]).to_csv(OUT/'iid59_drgp_summary.csv',index=False)
print(summary)
